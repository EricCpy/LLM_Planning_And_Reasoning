...
MODEL_METADATA = {
    ...
    LlmModel.LOCAL: ModelMetadata("local", 8192, cost_factor=0)
}
...
class AIStructuredResponseGeneratorBlock(Block):
    ...
    def llm_call(
        ...
        if provider == "openai":
            ...
            # Handle the local model call (e.g., for a locally hosted LLM API)
            url = "http://host.docker.internal:4891/v1/chat/completions"
            data = {
            ...
            if response.status_code == 200:
                ...
                raise ValueError(f"Local model error: {response.status_code} - {response.text}")
